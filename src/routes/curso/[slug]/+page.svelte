<script>
	...
	import { convertirTimestampAFecha } from '$lib/utils/timestampToDate';
	import { usuario } from '$lib/store';

	let cursoId = $page.params.slug;
	...
	onMount(async () => {
		...
		if (docSnap.exists()) {
			let fechaFormateada = convertirTimestampAFecha(docSnap.data().fecha);
			cursoData = { ...docSnap.data(), fecha: fechaFormateada };
		}
	});

	async function guardarCurso() {
		const documentoRef = doc(db, 'cursos', cursoId);
		try {
...
<section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
	...
	<h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">
		El curso ha sido creado desea publicarlo?
	</h2>
	<form on:submit|preventDefault={guardarCurso}>
		...
		<div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
			<FileField
				idDoc={cursoId}
				urlImagen={cursoData.publicidadImage}
				id="publicidadImage"
				tipoImagen="publicidadImage"
				name="publicidadImage"
				label="Imagen para publicidad"
				accept="image/"
			/>
			<FileField
				idDoc={cursoId}
				urlImagen={cursoData.certificadoImage}
				id="certificadoImage"
				tipoImagen="certificadoImage"
				name="certificadoImage"
				label="Imagen para certificado"
				accept="image/"
			/>
		</div>
